...
public class CreateShowUseCase {
    ...
    public CreateShowResponse execute(Long userId, CreateShowRequest request) {
        ...
        if (!event.isCreatedBy(userId)) {
            throw new ResponseStatusException(FORBIDDEN, "event owner mismatch");
        }

        Venue venue = venueRepository.findById(request.venueId())
                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "venue not found"));
        if (!venue.isCreatedBy(userId)) {
            throw new ResponseStatusException(FORBIDDEN, "venue owner mismatch");
        }

        Show show = showRepository.save(Show.schedule(event, request.startAt(), venue.getId(), userId));
