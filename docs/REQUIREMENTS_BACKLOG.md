...
| Common | REQ-COM-003 | 시간 표현 일관성 | ISO-8601, Asia/Seoul 기준으로 일관되게 반환한다 | P1 | 구현완료 | JVM/Jackson/Docker TZ Asia/Seoul |
...
| Common | REQ-COM-005 | 날짜별 파일 로깅 | 백엔드 로그를 `yyyyMMdd_back.log` 형식으로 보관하고 컨테이너 로그 디렉터리 권한을 보장한다 | P1 | 구현완료 | `LOG_PATH`, Docker `/app/logs` |
| Auth | REQ-AUTH-001 | 회원가입 | 이메일 unique, BCrypt 해시, 기본 USER role | P0 | 구현완료 | |
| Auth | REQ-AUTH-002 | 로그인 | email/password로 access/refresh 발급, Redis에 사용자별 최대 5개 토큰 세션 저장 | P0 | 구현완료 | 초과 시 가장 오래된 세션 제거 |
| Auth | REQ-AUTH-003 | JWT 인증 필터 | Bearer 토큰 검증 및 Redis access token 인덱스 확인 후 SecurityContext 세팅 | P0 | 구현완료 | |
| Auth | REQ-AUTH-004 | Refresh 재발급 | 현재 refresh로 access/refresh 동시 재발급, 토큰 회전 처리 | P0 | 구현완료 | Redis rotation |
| Auth | REQ-AUTH-005 | 로그아웃 | 요청 refresh token에 해당하는 Redis 토큰 세션 삭제 | P1 | 구현완료 | Redis revoke |
...
| Hold | REQ-HOLD-003 | 동시성 안전성 | 좌석 중복 홀드 불가 | P0 | 진행중 | Redis pre-lock + 요청 좌석 기준 row lock + `@Version` 적용 |
...
| Payment | REQ-PAY-002 | 결제 승인 처리 | 승인 후 PAID 전환 및 티켓 발급 | P0 | 구현완료 | amount 검증, 좌석별 티켓 발급 |
...
| Admin | REQ-ADM-002 | 공연장 좌석/구역 기준정보 등록 | venue 기준 `sections`와 구역에 속한 `seats`를 생성한다 | P0 | 구현완료 | 좌석 등록 시 sectionId 필수 |
| Admin | REQ-ADM-003 | 회차 판매 인벤토리 생성 | show 기준 `show_seats`를 생성한다 | P0 | 구현완료 | 구역 없는 좌석은 400 |
| Admin | REQ-ADM-004 | 이벤트/회차/판매단위 관리 | 이벤트, 회차, 좌석 판매 생성 | P0 | 구현완료 | |
